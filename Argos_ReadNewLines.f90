Program Argos
    !       Converts ARGOS binary data to Campbell measurements
    !       Sondy 5-30-95, Boulder 7-3-95
    !       Revised Sept 1, 1995
    !	Revise 30 April 2007 in Greenland
    !	Revised 1.2.2014 inEgg
    !       Revised  14.11.2014 Lugano
    !       Revised  1.7.2016 Birmensdorf
    !       test with new compiler 11.11.2014
    !       Revised 24.9.2018 Birmensdorf Derek Houtz
    !       Want to count lines read, so I add index+1 for every read
    !       and index-1 for every backspace
    ...
    index = 0
    !    loop for x data transmissions
    15   do 25 ii = 1, 10000000

        !        print *,ii

        !    read first character of id: 0(1463)
        111    read(1, 1001, end = 5000) (num(kk), kk = 1, 20)
        index = index + 1
        if(num(1).eq.'0'.and. num(2).eq.'1'.and.num(3).eq.'4'&
                .and.num(4).eq.'6'.and.num(5).eq.'3'.and.num(6).eq.' ') then

            read(1, 1001, end = 5000) (num(kk), kk = 1, 20)
            index = index + 1
            if(num(1).eq.' '.and.num(2).eq.' '.and.num(3).eq.' '.and.&
                    num(4).eq.' '.and.num(5).eq.' '.and.num(6).eq.' ') then
        ...
        !    check for line errors due to satellite transmission
        if(num(1).eq.' '.and.num(2).eq.' '       .and.num(3).eq.' '.and.&
                num(4).eq.' '.and.num(5).eq.' '.and.num(6).eq.' '.and.num(7)&
                .eq.'2'.and.num(8).eq.'0') then
            backspace(1)
            index = index - 1
            goto 40
        else
        ...
        !   data transmission of 16 data points
        do 50 i = 1, 16
        ...
        50   continue
        !	if(sta.eq.135796) nn=2
        !	if(sta.eq.107282) nn=3
        !	if(sta.eq.107283) nn=4
        !	if(sta.eq.135797) nn=5
        !	if(sta.eq.135798) nn=6
        !	if(sta.eq.107285) nn=7
        !	if(sta.eq.107284) nn=8
        ! Modified 13.9.2018 Derek Houtz, output to one file for uniformity
        ! Want single file to then sort and merge in MATLAB like goes data
    ...
    5000   continue
    ! We now add the number of lines processed to the previously loaded
